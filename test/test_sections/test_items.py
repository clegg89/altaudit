...
def test_items_estimated_ilvl_missing_offhand_with_2h(default_items_response):
    jack = Character('jack')
    ...
    mainhand = next(item for item in default_items_response['equipment']['equipped_items'] if item['slot']['type'] == 'MAIN_HAND')
    mainhand['inventory_type'] = { 'type' : 'TWOHWEAPON' }

    Section.items(jack, default_items_response, None, None)
    assert jack.estimated_ilvl == 405

def test_items_estimated_ilvl_missing_offhand_one_hand_weapon(default_items_response):
    jack = Character('jack')
    default_items_response['equipment']['equipped_items'].remove(next(item for item in default_items_response['equipment']['equipped_items'] if item['slot']['type'] == 'OFF_HAND'))
    mainhand = next(item for item in default_items_response['equipment']['equipped_items'] if item['slot']['type'] == 'MAIN_HAND')
    mainhand['inventory_type'] = { 'type' : 'WEAPON' }
