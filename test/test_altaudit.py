...
"""
Unit Tests for main altaudit entry point
"""
import pytest
...
def test_internal_get_metadata(mocker):
    ...
    result = altaudit.altaudit.get_metadata(mock_datetime)

    mock_datetime.utcnow.assert_called_once()
    assert result[0] == mock_datetime.utcnow.return_value
    assert result[1] == altaudit.altaudit.VERSION
...
def test_internal_get_all_character_info(mock_load_or_fetch, mock_get_char_data, mock_get_basic_info, mock_get_all_items, mock_get_azerite_info, mock_get_audit_info, mock_get_profession_info, mock_get_reputation_info, mocker):
    ...
    result = altaudit.altaudit._get_all_character_info(character, 'Now', 'Blizzard_API')

    mock_load_or_fetch.assert_has_calls(
            [mocker.call('classes.pkl', altaudit.altaudit.get_classes, 'Now', 'Blizzard_API'),
                mocker.call('races.pkl', altaudit.altaudit.get_races, 'Now', 'Blizzard_API')],
            any_order=True)
...
def fake_load_yaml(mocker, fake_tokens, fake_char_yaml, fake_server):
    fake = mocker.patch('altaudit.altaudit.load_yaml_file')
...
def test_altaudit_fetch_all_character_info(mock_blizzard_api, fake_tokens, fake_char_yaml, mock_get_all_character_info, mocker):
    characters = altaudit.utility.convert_to_char_list(fake_char_yaml)
    g = Counter()
    mock_datetime = mocker.MagicMock()
...
Actual: {}, {}, {}.""".format(characters, g.index, mock_blizzard_api, character, call_now, api))
    ...
    result = altaudit.fetch_all(fake_tokens, fake_char_yaml, mock_datetime)
...
def mock_fetch_all(mocker, fake_rows):
    mock = mocker.patch('altaudit.altaudit.fetch_all')
...
@pytest.fixture
def mock_csv(mocker):
    return mocker.patch('altaudit.altaudit.csv')

@pytest.fixture
def mock_sleep(mocker):
    return mocker.patch('altaudit.altaudit.time.sleep')

@pytest.fixture
def mock_os(mocker):
    return mocker.patch('altaudit.altaudit.os.system')

@pytest.fixture
def mock_altaudit_open(mocker):
    return mocker.patch('altaudit.altaudit.open', mocker.mock_open())

@pytest.fixture
def mock_get_metadata(mocker):
    mock = mocker.patch('altaudit.altaudit.get_metadata')
    mock.return_value = ['The Date', 'Version']

    return mock

def test_main_load_config_file(mock_fetch_all, mock_csv, mock_sleep, mock_os,
        fake_load_yaml, mock_altaudit_open):
...
def test_main_get_metadata_called_correctly(mock_get_metadata, mock_fetch_all, mock_csv, mock_sleep, mock_os,
        fake_load_yaml, mock_altaudit_open, mocker):
...
def test_main_open_called_properly(mock_fetch_all, mock_csv, mock_sleep, mock_os,
    ...
    assert mock_altaudit_open.call_count == 2
    assert mock_altaudit_open.call_args_list == [open_call, open_call]
...
def test_main_csv_called_properly(mock_get_metadata, mock_fetch_all, fake_rows, mock_csv, mock_sleep, mock_os,
    ...
    writer_call = mocker.call(mock_altaudit_open.return_value)
...
def test_main_open_sleep_called_properly(mock_fetch_all, mock_csv, mock_sleep, mock_os,
    ...
    manager.attach_mock(mock_altaudit_open, 'open')
